S.editor.resources = {
    ...
    select: function (path, filetypes, multiselect, title, buttonTitle, callback) {
        ...
        S.popup.show(title, $('#template_resources_popup').html().replace('##button-title##', buttonTitle));
        $('.popup').css({ 'width': 'calc(100% - 30px)' });
        $(window).on('resize', resizeResources);

        function resizeResources() {
            var win = S.window.pos();
            $('.popup .resources-list').css({ 'max-height': (win.h - 200) + 'px' });
        }
        resizeResources();
        ...
        S.ajax.post('PageResources/Render', { path: path, filetypes:filetypes },
            function (d) {
                $('.popup .resources-list').html(d);
                $('.popup .img .close-btn').each(a => {
                    $(a).attr('onclick', $(a).attr('onclick').replace('this)', 'this, \'' + path + '\')'));
                });
                $('.popup .img').prepend($('#template_resource_selected').html());
                $('.popup .img').on('click', (e) => {
                    var target = e.target;
                ...
                $('.popup .uploader').on('click', uploader.click);
                $('.popup .apply').on('click', () => {
                    S.popup.hide();
